 ...
 * Reused from the wizard-timer project — unchanged apart from dropping its theme notes.
 *
 ...
 * makes persistence.ts able to resume a running timer for free.
 */
...
export function start(state: TimerState, now: number): TimerState {
  ...
  const remaining = state.status === 'finished' ? state.durationMs : remainingMs(state, now);
